...
import { GetResultCompilerService } from '../../modules/Exam/Application/GetResultCompiler';
...
import {
  GetExamRequest,
  GetExerciseCode,
  PostStudentAnswersOfExamRequest,
} from './ExamRequest';
...
export class ExamController {
  constructor(
    ...
    private readonly getResultCompilerService: GetResultCompilerService,
  ) {}

  @Post('/compiler')
  async getResultCompiler(@Body() request: GetExerciseCode, @Res() res: any) {
    try {
      const result = await this.getResultCompilerService.handle(
        request.code,
        request.language,
      );
  ...
  async getExamByThemeId(
    ...
    try {
      const examResponse: GetExamResponse = await this.getExamService.handle(
  ...
  async insertStudentAnswersOfExam(
    ...
    try {
      await this.insertStudentAnswerByExamService.handle(
        StudentAnswersByExamMapper.convertPostStudentAnswersOfExamToStudentExam(
          postStudentAnswersOfExamRequest,
        ),
      );
